...
public class PromptFormatter {
    ...
    private static final String TRADE_SYSTEM_PROMPT = """
            ...
            1. 股票名称：原样保留文本中的股票名称、ETF 或带有 *ST 等前缀的标的；若名称附带市场后缀（如 .SH/.SZ）须去除；不要在此处输出 6 位数字代码；
            2. 买卖方向：严格归一化为 "BUY"（买入）或 "SELL"（卖出）；
            3. 成交价格：精确读取浮点数，保留完整小数位（如 14.880）；
            4. 成交数量：必须为正整数；
            5. 成交时间：严格格式化为 "YYYY-MM-DD HH:mm:ss"。
               - 截图中只有年月（如 2026-09）时，默认填充为该月 1 日零点（如 "2026-09-01 00:00:00"）；
               - 只有年月日（如 2026-08-21）时，时间部分默认填充为 "00:00:00"；
               - 严禁因缺少具体日、时、分、秒而拒绝提取。
            【核心执行铁律】只要单条记录同时具备【股票名称、大于 0 的成交价格、大于 0 的成交数量、买卖方向】，就必须提取为有效流水！绝不允许无故输出 []。
            【噪声清洗】忽略界面控件文字（如“去开启”“成交汇总”“筛选”等）；忽略价格为 0 或非二级市场买卖的记录（如“四方配号”“中签”等）。
            输出格式要求：必须且仅输出严格的 JSON 二维数组（严禁包含任何 Markdown 标记或多余文字）：
            [["股票名称","BUY/SELL",成交价格,成交数量,"成交时间"]]
            文本中没有任何有效成交流水时输出 []。
            """;
    ...
    private static final String TRADE_STRICT_REVIEW = """
            ...
            1. 逐字校对股票名称与数字，警惕 OCR 常见的 0/6/8、1/7 混淆、小数点粘连与断行错位；
