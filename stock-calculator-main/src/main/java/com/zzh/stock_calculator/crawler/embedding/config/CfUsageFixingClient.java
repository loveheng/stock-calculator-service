...
/**
 * CF Workers AI 兼容垫片（设计偏离上报后用户拍板的 Option A，2026-09-08）。
 *
 ...
 * 回填合成 Usage(0,0)（Spring AI 仅透传该值到响应元数据，不参与向量逻辑），
 * 其余 27 个方法一行转发给真实 client。openai-java 升级新增接口方法时，
 * 编译器强制要求补齐转发，升级安全。
 *
 * <p>探针采用 try/catch response.usage()（与 Spring AI 内部调用同一访问器），
 * 覆盖 getRequired 抛错的所有形态（缺失 / null / 类型不符），而非猜测 JsonField 内部状态。
 */
