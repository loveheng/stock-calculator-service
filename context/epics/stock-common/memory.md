...
4. **contract 纯 POJO 约束会被破坏**（D9/R2）：管道类依赖 Spring/RestClient/PDFBox，下沉违反 contract「禁引 Spring 类型」定位，并扩大两个 native image 的AOT 配置面。
...
- [断点] 下一步：main 侧 8 处迁移过渡副本加注释标记（§5 决定动作第 1 项）
