...
## 追加区
- [main 构建] Mockito 报 "Cannot instrument interface ... Unresolved compilation problem: List cannot be resolved"（源码实际缺 import） ➔ IDE 的 ECJ 把带编译错误的 .class 写进 target/classes，Maven 增量编译见 .class 比 .java 新而跳过重编，javac 从未真正编译该文件 ➔ 改动 Repository/接口后跑测试遇 "Unresolved compilation problem" 先补真实编译错误再 clean test，勿信增量产物 (Ref: misc)
- [data-mq] 公告嵌入任务被 worker 消费但向量零落库、队列零积压零死信，表象「一切正常」 ➔ EmbeddingComputeWorker kind 门控白名单缺 announcement（业务性 skip 后 ack 丢弃，静默黑洞：无 retry 无 dead 无任何错误痕迹） ➔ MQ 消费端新增消息 kind 必须与计算/处理端白名单同轮接线，两头各留 TODO 必挂；排障时「队列干净」不等于链路通，要盯消费端 skip 日志与端到端产物 (Ref: misc)
- [announcement] 公告向量 SQL 正常执行却检索恒空、回查零命中，且无任何报错 ➔ 写入侧 metadata.announcementId 误存内部自增 id（getId），读取方按 CNINFO 标识（getAnnouncementId）查询，键语义写读分裂 ➔ metadata 键值必须以「读取方的查询口径」为准并加端到端断言（单测 mock 层面写入读出自洽、跨端键义错位测不出）；「键名对但值域错」的 bug 只有真库联调能暴露，插入前先人工核对一行 metadata 样本 (Ref: misc)
- [search] SSE /search/composite 检索正常出 meta 后紧跟 error 503「综合摘要服务暂不可用」 ➔ LlmChainRouter 全渠道不可用回落 fallback 模板：运行进程环境 GEMINI_API_KEY 缺失（isAvailable 判否跳过）+ GROQ_API_KEY=your_groq_api_key 占位符（非空校验通过但 Groq 调用期 401），isDegradedResponse 识别模板后映射 503 ➔ composite 503 先查**运行进程**实拿的渠道 key（/proc/进程号/environ），占位符假 key 能通过 isAvailable 非空校验、只在调用期 401；配置修复以进程实拿环境为准，勿信 .env 文件字面 (Ref: misc)
- [main-runtime] main 由 IDE 托管无法稳定携带 AI 出口代理，且 JVM 设全局代理后会劫持 docker 主机名的内部 HTTP（lavinmq 管理 API 503 → BROKER_UNREACHABLE 假告警） ➔ 代理经 JAVA_TOOL_OPTIONS 注入时 http.nonProxyHosts 必须排除内部主机名（默认白名单仅 localhost|127.*|[::1]，docker 容器别名 lavinmq 不覆盖） ➔ 重启走 /tmp/scs-main-launch.py：先快照在跑 pid 的 environ/cmdline（用完即删防密钥留存），.env 覆盖同名变量，setsid 脱离会话拉起，日志 /tmp/scs-main.log；main 已脱离 IDE 托管，IDE 再起需自配代理 VM 参数与最新 .env (Ref: misc)
- [search] 公告关键词检索带日期 500（could not determine data type of parameter $7），不传日期反而正常 ➔ JPQL (:param IS NULL OR col>=:param) 谓词：null 绑定经 setNull 带类型 OK，非 null LocalDate 经 setObject 无类型下发，PG 对「参数 IS NULL」裸位置推断不出类型（42P18）；「列 IS NULL OR 列比较参数」形态安全（参数处于比较上下文可推断） ➔ 日期条件禁止写 :param IS NULL 谓词，Service 按条件成立与否分流到带/不带谓词的方法（AnnouncementRepository 四变体先例）；同类模式全仓排查只此一处危险（ClsArticleRepository 同形但 Long typed 绑定实测安全） (Ref: misc)
