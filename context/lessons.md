...
- [字典校验] 同域多端点复制同一校验但只修一处：klines 已改 existsBySixDigit（6 位尾匹配），compute/ask/monitor 仍用 existsByCode（裸 6 位码 existsById 永远落空）→ 三端点对任何合法代码返回 400「未收录」，各自单测 mock 自洽测不出，E2E 才暴露 ➔ 跨端点同形态校验必须统一入口（或一个工具方法），修一处即 grep 全量同形态调用点对齐；回归测试必须含「命中通过」正向断言——只测拒绝分支（false → 400）测不出「永远落空」型缺陷 (Ref: free-canvas)
- [LLM 网关] KG 抽取任务大面积 TRANSIENT 失败、主服务《新闻联播》数据长期不前滑（先报 empty llm response，放大 token 后变成每隔整 60s 一次 Request failed）➔ openai-mini 档用 step-3.7-flash 属带 thinking 模型且 reasoning 计入 max_tokens：默认 8096 被思考吃光返回 finish_reason=length + content 空串；放大到 32768 后单次 ~13k completion tokens 又超 60s 客户端读超时被掐 ➔ thinking 型模型必须同时放宽 max-tokens（≥32k）与 timeout（≥240s）；判空 branch 前先查 finish_reason=length 排除配额原因；「精确等间隔失败的调用」= 客户端超时在掐而非渠道抖动；修复后旧 FAILED 行靠 FAILED 治愈回 PENDING 与 backfill 最旧优先自动追平，无需手工补发 (Ref: news-kg)
- [mcp] mcp 服务端 @Tool bean 注册了但 MCP listTools 不出现（工具调用 404/静默降级） ➔ 新增 @Tool 工具类只建了 @Component bean，漏挂 McpToolConfig.toolCallbackProvider 的 toolObjects 列表（MethodToolCallbackProvider 只暴露显式列出的对象，不自动扫描）——fetch_realtime_quote 孤儿两月、stock_radar_batch 差点重蹈 ➔ mcp 模块新增工具三件套缺一不可：@Component + McpToolConfig 挂 provider + ToolRegistry seed 登记；冒烟以启动日志 "Registered tools: N" 计数核对 (Ref: misc)
