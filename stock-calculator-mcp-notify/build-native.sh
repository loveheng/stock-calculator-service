#!/usr/bin/env bash
# ============================================================================
# Stock MCP Notify Service (:18082) — GraalVM Native 直接编译脚本
#
...
#   2. 独立库 stock_mcp（PG）+ RabbitMQ：启动冒烟需真实连库（同 main 套路，
#      CI 由 workflow 的 postgres service 预灌 schema；本地复用 compose 常驻实例）
#   3. RabbitMQ 连接 fail-fast：构建期注入 dummy 防止 process-aot 实例化单例时
#      意外触发（运行期值仍从环境变量取）
...
# ============================================================================
set -e
cd "$(dirname "$0")"   # 进入 stock-calculator-mcp-notify/

BINARY_NAME="stock-mcp-notify-service"
...
echo "════════════════════════════════════════════════════════════════"
echo " Stock MCP Notify Service GraalVM Native 编译"
...
if [ "$SKIP_PKG" != "--no-pkg" ]; then
  echo "█████ 步骤 0/4: install 父 POM + contract + llm + jpa + main（notify 单独编译的解析前提；P2 起 main 传递依赖 stock-calculator-jpa 与 llm）..."
...
# AOT 产物校验
if [ ! -f target/spring-aot/main/classes/com/zzh/stock_calculator/notify/NotifyApplication__ApplicationContextInitializer.class ]; then
...
echo "█████ 步骤 3/4: native-image（约 8~15 分钟，日志 /tmp/ni-notify-build.log）..."
# 官方 reachability-metadata 仓库接管 hikari/hibernate 反射缺口（试点已验证）
if ! native-image \
  -cp "$CP" \
  -H:Class=com.zzh.stock_calculator.notify.NotifyApplication \
  ...
  -H:IncludeResources='notify-schema.sql' \
  ...
  -H:NumberOfThreads=8 \
  > /tmp/ni-notify-build.log 2>&1; then
  echo "❌ native-image 编译失败，日志末尾 60 行："
  tail -60 /tmp/ni-notify-build.log
  exit 1
fi
...
SMOKE_LOG=/tmp/notify-smoke.log
"target/$BINARY_NAME" > "$SMOKE_LOG" 2>&1 &
SMOKE_PID=$!
OK=""
for i in $(seq 1 30); do
  if curl -s -o /dev/null http://localhost:18082/ 2>/dev/null; then OK=1; break; fi
