#!/usr/bin/env bash
# ============================================================================
# Stock MCP Orchestration Service (:18083) — GraalVM Native 直接编译脚本
#
# 参照 stock-calculator-mcp/build-native.sh（同套路），适配点：
#   1. 依赖 main 模块 artifact（stock-calculator-service）+ contract，
#      先 install 父 POM + contract + main 到 ~/.m2
#   2. 独立库 stock_mcp（PG）+ RabbitMQ + MCP client（连 :18081 经纪人）：
#      启动冒烟需真实连库（同 main 套路，CI 由 workflow 的 postgres service
#      预灌 schema；本地复用 compose 常驻实例）
#   3. RabbitMQ/LLM/CF 三键默认空值：构建期注入 dummy 防 process-aot 实例化
#      单例时意外触发 fail-fast（运行期值仍从环境变量取）
#   4. 复用 gen-native-metadata.py（jboss-logging 动态 logger + openai any-setter
#      反射注册；脚本按自身所在目录的 target/cp.txt 扫描，模块无关直接拷贝）
#
# 用法:
#   ./build-native.sh            # 完整构建（install 依赖 + compile + AOT + native-image + 冒烟）
#   ./build-native.sh --no-pkg   # 跳过 maven 编译，复用已有 target/ 产物
# ============================================================================
set -e
cd "$(dirname "$0")"   # 进入 stock-calculator-orchestration/

BINARY_NAME="stock-mcp-orchestration-service"

# ---------------- 步骤 0: 锁定 GraalVM 25.0.x（与 main/data 同套路） ----------------
GRAALVM_HOME=""
...
echo "════════════════════════════════════════════════════════════════"
echo " Stock MCP Orchestration Service GraalVM Native 编译"
echo "   native-image = $(native-image --version 2>&1 | head -1)"
echo "════════════════════════════════════════════════════════════════"

# 构建期 dummy 凭据（process-aot 实例化全部单例；运行期值仍从环境变量取）
export SPRING_APPLICATION_JSON='{
  "spring": {"rabbitmq": {"host": "localhost", "port": 5672, "username": "build-time-dummy", "password": "build-time-dummy"}},
  "ai": {"tiers": {"openai-max": {"base-url": "http://build-time.invalid", "api-key": "build-time-dummy", "model": "build-time-dummy"}},
...
if [ "$SKIP_PKG" != "--no-pkg" ]; then
  echo "█████ 步骤 0/4: install 父 POM + contract + llm + jpa + main（orchestration 单独编译的解析前提；P2 起 main 传递依赖 stock-calculator-jpa）..."
...
# AOT 产物校验
if [ ! -f target/spring-aot/main/classes/com/zzh/stock_calculator/orchestration/OrchestrationApplication__ApplicationContextInitializer.class ]; then
...
if ! native-image \
  ...
  -H:NumberOfThreads=8 \
  > /tmp/ni-orch-build.log 2>&1; then
  echo "❌ native-image 编译失败，日志末尾 60 行："
  tail -60 /tmp/ni-orch-build.log
  exit 1
fi

# 剥离全部符号（与 main/data 同款：--strip-all 各环境结果确定）
if command -v objcopy >/dev/null 2>&1; then
  echo "█████ 剥离全部符号（objcopy --strip-all）..."
  BEFORE_SIZE="$(du -h "target/$BINARY_NAME" | cut -f1)"
  objcopy --strip-all "target/$BINARY_NAME"
...
for i in $(seq 1 30); do
  if curl -s -o /dev/null http://localhost:18083/ 2>/dev/null; then OK=1; break; fi
  if ! kill -0 "$SMOKE_PID" 2>/dev/null; then break; fi
  sleep 1
done
if [ -z "$OK" ]; then
  echo "❌ 冒烟失败（启动探活 30s 超时/进程退出），日志末尾 40 行："
  tail -40 "$SMOKE_LOG"
  kill "$SMOKE_PID" 2>/dev/null || true
  exit 1
fi
kill "$SMOKE_PID" 2>/dev/null || true
echo "✅ 冒烟通过（:18083 HTTP 探活）"
