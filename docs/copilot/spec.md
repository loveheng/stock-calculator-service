...
> **现状注记（2026-09-12）**：本文 LLM 渠道与端点行为描述部分过时——实际聊天渠道为 **copilot 专用 DeepSeek 付费渠道**（原 `copilot.llm.deepseek.*`，见 `docs/copilot/api.md` 与 `DeepSeekConfig`），「复用 llm 域容灾链」从未实现；SSE 流式提问、Prompt 模板管理（`/api/copilot/prompt/templates`）与 custom-stats `taskType` 集成为既有代码但本文未覆盖。冲突处以 `docs/copilot/api.md` 与代码为准。
> **现状注记（2026-09-24）**：装配设施已换轨 stock-calculator-llm——模型走 `LlmRegistry`（`ai.tiers.openai-max` 档，env 同源 `OPENAI_MAX_*`），`DeepSeekConfig/DeepSeekProperties` 已删除；渠道隔离决策不变，详见 `docs/architecture/llm-module.md`。
