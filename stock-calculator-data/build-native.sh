...
#   3. 条件装配（collector/worker/ingest）在构建期钉死为三角色全开（AOT 固化条件
#      评估，运行期不可切换，R1 教训）。单镜像多副本的「恰一个」语义全部由 MQ
#      协议仲裁，打包不再承载角色区分：
...
BINARY_NAME="stock-calculator-data-service"
...
if [ "$SKIP_PKG" != "--no-pkg" ]; then
  ...
  ../mvnw -f .. install -pl stock-calculator-contract,stock-calculator-llm -q -DskipTests
...
echo "█████ 步骤 2.5/4: 生成 openai SDK 反射元数据（共享脚本 gen-openai-metadata.py）..."
...
# 剥离全部符号（--strip-all = 符号表 + DWARF 调试段；只影响 gdb 符号化，不影响
# 运行——Java 异常堆栈走运行期自身元数据）。不用 --strip-debug：不同 binutils 上
# 是否连带剥 .symtab 行为不一致，--strip-all 各环境结果确定。
# 后续冒烟与镜像打包用的都是剥离后的最终产物
