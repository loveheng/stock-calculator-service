...
/**
 * collector 门控验证（D4：副本恒=1；worker 部署不装配拉取消费者）。
...
class CollectorGateTest {
    ...
    @EnableConfigurationProperties(PullLoopProperties.class)
    static class Deps {
        @Bean
    ...
    void gateMissingDefaultsToDisabled() {
        ...
        runner.run(context -> assertThat(context).doesNotHaveBean("clsPullConsumer"));
    }
}
